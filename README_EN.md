...
<b> Go from idea to presentation in minutes. No tedious formatting, just natural language modifications—step into true "Vibe PPT" </b>
...
Have you ever found yourself in this dilemma: a presentation is due tomorrow, but your PPT is still a blank slate; you have countless brilliant ideas in your head, but your enthusiasm is drained by the tedious layout and design?

I (we) long to quickly create professional and well-designed presentations. While traditional AI PPT generation apps generally meet the need for "speed," they still suffer from the following issues:
...
- 3️⃣ The finished products look similar, with severe homogenization.
- 4️⃣ Low material quality and a lack of specificity.
- 5️⃣ Disjointed text and image layout, with poor design aesthetics.

These flaws make it difficult for traditional AI PPT generators to simultaneously satisfy our two major needs: "speed" and "beauty." Even those claiming to be "Vibe PPT" are, in my eyes, far from being "Vibe" enough.

However, the emergence of the nano banana🍌 model has changed everything. I tried using 🍌pro for PPT page generation and found that the results were excellent in terms of quality, aesthetics, and consistency. It can accurately render almost all the text requested in the prompts and follow the style of reference images. So, why not build a native "Vibe PPT" application based on 🍌pro?
...
3. **Educators**: Quickly convert teaching content into illustrated lesson plan PPTs, enhancing classroom effectiveness.
...
<p>
  <b>🎯 Goal: Lower the barrier to entry for PPT creation, enabling everyone to quickly create beautiful and professional presentations.</b>
</p>
...
- **Natural language editing**: Supports modifying outlines or descriptions via "Vibe" (e.g., "change page three to a case study"), with AI responding and adjusting in real-time.
...
No longer restricted by complex menus and buttons, issue editing commands directly using **natural language**.
- **Partial Redrawing**: Make conversational modifications to specific areas (e.g., "Change this chart to a pie chart").
...
- **Perfect Adaptation**: Default 16:9 aspect ratio, no manual layout adjustments required, ready for presentation.
...
| Secondary Editing | Not supported | **Selection-based editing + Verbal editing** |
| Asset Addition | Cannot add after generation | **Freely add after generation** |
| Export Formats | Supports PDF export only | **Export to PDF, (editable) PPTX** |
| Watermark | Watermarks in free version | **No watermark, freely add or delete elements** |
...
- 【2-9】：
  * New Features
    * Support pasting images on the homepage, outline, and description cards for immediate recognition, providing a better interactive experience.
  ...
  * Fixes and Experience Optimizations
    * Fixed export-related 500 errors, reference file association timing, outline/page data misalignment, task polling errors, infinite polling in description generation, image preview memory leaks, and partial failure handling for batch deletions.
    * Optimized format example hints, HTTP error messages, Modal closing experience, cleaned up old project localStorage, and removed redundant prompts for first-time project creation.
    * Various other optimizations and fixes.
- 【1-4】 : v0.4.0 Release: Comprehensive upgrade to editable pptx export:
  * Supports maximum restoration of font size, color, bold, and other styles for text within images;
  * Supports recognizing text content within tables;
...
- 【12-27】: Added support for a text-only template mode and high-quality text presets; PPT page styles can now be controlled via pure text descriptions.
...
| ✅ Completed | Area-selection Vibe verbal editing for single slides |
...
| 🔄 In Progress | Support for exporting editable .pptx with multi-layer, precise background removal |
...
1. Deploy and launch this application with one click via Rainyun (New users enjoy a 15-day free trial + double credit on first recharge policy).
...
If you are using **Windows or macOS**, please install **Docker Desktop** first and ensure that Docker is running (Windows users can check the system tray icon; macOS users can check the menu bar icon), then follow the same steps in the documentation.
...
> **The LLM API in this project follows the AIHubMix platform standard. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain an API key to reduce migration costs.**<br>
...
**Use the new editable export configuration method for better results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (there is a generous free usage quota). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for details.
...
> **The Large Language Model (LLM) interfaces in this project follow the AIHubMix platform standard. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain an API key to reduce migration costs.** 
```env
```
...
# You can modify this variable to control the backend service port
...
- **Concurrency Processing**: ThreadPoolExecutor
...
1. **Generated page text is garbled or blurry**
    - Choose a higher output resolution (OpenAI format may not support increasing resolution; Gemini format is recommended). Tests show that upgrading from 1k to 2k resolution significantly improves text rendering quality.
...
2. **Poor results when exporting editable PPT, such as overlapping text or missing styles**
    - In 90% of cases, this is due to an issue with the API configuration. Please refer to [issue 121](https://github.com/Anionex/banana-slides/issues/121) for troubleshooting and solutions.
...
4. **503 error or Retry Error when generating content**
    - Check the Docker backend logs using the commands provided in the README to locate the detailed error for the 503 issue. This is usually caused by incorrect model configuration.
...
5. **Why doesn't the API Key take effect after setting it in .env?**
    ...
    - If parameters were previously configured in the web settings page, they will override the `.env` parameters. You can return to the `.env` settings by clicking "Restore Default Settings".
...
This project is open-sourced under the **GNU Affero General Public License v3.0 (AGPL-3.0)**. It can be freely used for non-commercial purposes such as personal learning, research, testing, education, or non-profit scientific research activities;
<details> 
<summary> Details </summary>
A Commercial License is required for commercial use (for example: closed-source use, private deployment and delivery, integrating this project into closed-source products, or providing services without disclosing the corresponding source code). Please contact the author: anionex@qq.com
...
<details>
  ...
  "A global multi-model API service aggregator. Enjoy safe and stable services with access to the world's latest models within 72 hours at a lower price."
</details>
...
Open source is not easy 🙏 If this project is valuable to you, please consider buying the developer a coffee ☕️
